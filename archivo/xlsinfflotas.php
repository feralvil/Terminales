...
if ($permiso > 1){
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("D3", $thorg);
    $objPHPExcel->getActiveSheet()->setCellValue("E3", $thterm);
    $objPHPExcel->getActiveSheet()->setCellValue("F3", $thtbase);
    $objPHPExcel->getActiveSheet()->setCellValue("G3", $thtmov);
    $objPHPExcel->getActiveSheet()->setCellValue("H3", $thport);
    $objPHPExcel->getActiveSheet()->setCellValue("I3", $thdesp);
    $objPHPExcel->getActiveSheet()->getStyle('A3:I3')->applyFromArray($estiloTh);
    ...
    for ($i = 0; $i < $nflotas; $i++){
        ...
        $objPHPExcel->getActiveSheet()->setCellValue("D" . $fila, $flota['ORGANIZACION']);
        //datos de la tabla Terminales
        // Tipos de termninales
        $tipos = array("%", "F", "M%", "P%", "D");
        $nterm = array(0, 0, 0, 0, 0);
        $columna = array('E', 'F', 'G', 'H', 'I');
        ...
        if (($i % 2) == 1){
            $objPHPExcel->getActiveSheet()->getStyle('A' . $fila . ':I' . $fila)->applyFromArray($estiloRelleno);
        }
    }
    $objPHPExcel->getActiveSheet()->getStyle('A3:I' . $fila)->applyFromArray($estiloCelda);
    $fila++;
    $fila++;
    $objPHPExcel->getActiveSheet()->setCellValue("A" . $fila, $totales . " - " . $nflotas);
