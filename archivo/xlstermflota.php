...
$fichero = "plantillas/termflota_$idioma.xls";
...
if ($nflota > 0){
    ...
    // Cumplimentamos los datos de los cuadros finales
    $fila = 28;
    if ($nterm > 4){
        $fila = $fila + $nterm - 4;
    }
    if ($idc > 0){
        $objPHPExcel->getActiveSheet()->setCellValue("G$fila", $row_contacto["NOMBRE"]);
    }
    $objPHPExcel->getActiveSheet()->setCellValue("L$fila", "Javier Gimeno Fleta");
    $fila = $fila + 4;
