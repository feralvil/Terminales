...
<?php
    ...
    while (($i < $numfilas)&&(!$ctec)){
        $celda = "A$i";
        if ($objPHPExcel->getActiveSheet()->getCell($celda)->getValue() == "CONTACTO TECNICO:"){
            $ctec = true;
        }
        $i++;
    }
    ...
    <table>
        <tr>
            ...
            <form name="detflota" action="update_flota.php" method="POST">
                ...
                <input name="idflota" type="hidden" value="<?php echo $idflota; ?>">
                <td class="borde">
                    <input type='image' name='nueva' src='imagenes/atras.png' alt='<?php echo $botdetalle; ?>' title="<?php echo $botdetalle; ?>"><br><?php echo $botdetalle
            . " de Flota"; ?>
                </td>
            </form>
        </tr>
