...
<html>
    <head>
        ...
        <script type="text/javascript">
            // Funciones JQuery
            $(function(){
                // Cargar el div con los datos de los terminales de la actuación:
                $("#datosact").load("sqlactuacion.php",$("form#actterm").serializeArray());
                
                // Cargar el div con los datos de las acciones a realizar:
                $("#newacterm").load("sqlacttadd.php");
                
                // Click en el botón de alta:
                $("div#newacterm").on("click", "a#actalta", function(){
                    $("#newacterm").load("sqlacttalta.php",$("form#actterm").serializeArray());
                });
                
                // Click en el botón de baja:
                $("div#newacterm").on("click", "a#actbaja", function(){
                    $("#tactissi").val("baja");
                    $("#newacterm").load("sqlacttissi.php",$("form#actterm").serializeArray());
                });
                
                // Click en el botón de búsqueda de ISSI:
                $("div#newacterm").on("click", "a#consultaissi", function(){
                    $("#newacterm").load("sqlacttissi.php",$("form#buscaissi").serializeArray());
                });
                
                // Click en el botón de modificar:
                $("div#newacterm").on("click", "a#actmod", function(){
                    $("#tactissi").val("mod");
                    $("#newacterm").load("sqlacttissi.php",$("form#actterm").serializeArray());
                });
                
                // Click al seleccionar terminal para modificar:
                $("div#newacterm").on("click", "a.modterm", function(){
                    var idterm = $(this).closest("td").children("input#termsel").val();
                    $("input#idterm").val(idterm);
                    $("#newacterm").load("sqlacttmod.php",$("form#datosterm").serializeArray());
                });
                
                // Click al seleccionar terminal para baja:
                $("div#newacterm").on("click", "a.bajaterm", function(){
                    var idterm = $(this).closest("td").children("input#termsel").val();
                    var issiterm = $(this).closest("td").children("input#issisel").val();
                    $("input#idterm").val(idterm);
                    if (confirm('<?php echo $confbaja;?>' + issiterm)) {
                        $("#newacterm").load("sqlacttupd.php",$("form#datosterm").serializeArray());
                    }
                });
                
                // Click en el botón de editar campo:
                $("div#newacterm").on("click", "a#editar", function(){
                    $(this).next("div#edicion").removeClass('oculto');
                    var valorig = $(this).next("div#edicion").children("select").val();
                    $(this).next("div#edicion").children("input#original").val(valorig);
                    $(this).addClass('invisible');
                });
                
                // Click en el botón de deshacer editar campo:
                $("div#newacterm").on("click", "a#borrar", function(){
                    $(this).closest("div#edicion").addClass('oculto');
                    $(this).prev("input[type='text']").val("");
                    var valorig = $(this).closest("div#edicion").children("input#original").val();
                    $(this).prev("select").val(valorig);
                    $(this).closest("td").children("a#editar").removeClass('invisible');
                });
                
                // Click en el botón de guardar:
                $("div#newacterm").on("click", "a#guardar", function(){
                    $("#newacterm").load("sqlacttupd.php",$("form#datosterm").serializeArray());
                });
                
                // Click en el botón de cancelar:
                $("div#newacterm").on("click", "a#cancelar", function(){
                    $("#newacterm").load("sqlacttadd.php");
                });
...
<?php
    ...
    $sql_actterm = "SELECT * FROM actterminales WHERE ACTUACION_ID='$idactuacion'";
...
?>
        ...
        <table>
            ...
            <tr class="filapar">
                ...
                <th><?php echo $mail; ?></th>
                <td><?php echo $row_actuacion["MAIL"]; ?></td>
            </tr>
        </table>
        <h2><?php echo $h2actuacion; ?></h2>
        <table>
            <tr>
                <th class="t4c"><?php echo $referencia; ?></th>
                <th class="t4c"><?php echo $fechaaper; ?></th>
                <th class="t4c"><?php echo $fechaaprob; ?></th>
                <th class="t4c"><?php echo $fechacierre; ?></th>
            </tr>
            <tr>
                <td><?php echo $row_actuacion["REFERENCIA"]; ?></td>
                <td><?php echo $row_actuacion["FPETICION"]; ?></td>
                <td><?php echo $row_actuacion["FAUTORIZACION"];?></td>
                <td><?php echo $row_actuacion["FCIERRE"];?></td>
            </tr>
        </table>
        <div id="datosact">
        </div>
        ...
        <form name="actterm" id="actterm" action="sqlactuacion.php" method="POST">
            <input type="hidden" name="idactuacion" value="<?php echo $idactuacion;?>">
            <input type="hidden" name="idflota" value="<?php echo $row_actuacion["FLOTA_ID"];?>">
            <input type="hidden" name="tactissi" id="tactissi" value="nada">
        </form>
