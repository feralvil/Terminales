...
<form name="buscaissi" id="buscaissi" action="sqlacttissi.php" method="POST">
    ...
    <input type="text" name="issibusca" id="issibusca" value="<?php echo $issibusca ;?>" size="10">
    &nbsp;
...
?>
            ...
            <form name="datosterm" id="datosterm" action="<?php echo $enlace ;?>" method="POST">
                <input type="hidden" name="idactuacion" value="<?php echo $idactuacion; ?>" />
                <input type="hidden" name="tactissi" value="<?php echo $tactissi ;?>" />
                <input type="hidden" name="tipoact" value="BAJA" />
                <input type="hidden" name="origen" id="origen" value="nueva" />
                <input type="hidden" name="idterm" id="idterm" value="nada" />
                <table>
                    <tr>
                        <th>Seleccionar</th>
...
<?php
                    for ($i = 0; $i < $nissi; $i++){
                        ...
                        $classlink = $tactissi."term";
?>
                        <tr<?php if (($i % 2) == 1) echo " class='filapar'" ?>>
                            <td class="centro">
                                <input type="hidden" name="termsel" id="termsel" value="<?php echo $row_term["ID"] ;?>" />
                                <input type="hidden" name="issisel" id="issisel" value="<?php echo $row_term["ISSI"] ;?>" />
                                <a href="#" class="<?php echo $classlink ;?>" id="<?php echo $idterm ;?>"><img src="imagenes/ir.png" alt="Seleccionar terminal" title="Seleccionar terminal"></a>
                            </td>
