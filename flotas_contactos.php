...
<html>
    ...
    <body>
        ...
        if ($permiso == 2){
            ...
            $nomcont = array(
                'RESPONSABLE' => 'Responsable de Flota', 'CONTACTO1' => 'Contacto 1', 
                'CONTACTO2' => 'Contacto 2', 'CONTACTO3' => 'Contacto 3', 
                'INCID1' => 'Contacto 1 para Incidencias', 'INCID2' => 'Contacto 2 para Incidencias', 
                'INCID3' => 'Contacto 3 para Incidencias', 'INCID4' => 'Contacto 4 para Incidencias'
            );
            if (isset ($update)){
                if ($update == "KO"){
                    $clase = "flashko";
        ...
        ?>
            ...
            <form name="formFlotas" id="formFlotas" action="flotas_contactos.php" method="post">
            <table>
                <tr>
                    <td class="borde">
                    ...
                    <td class="borde">
                        <select name="contactsel" id="selContacto" onchange="formFlotas.submit();">
                            <option value="NN">Seleccionar Tipo de Contacto</option>
                        <?php
            ...
            if ($nflotas > 0){
            ?>
                <form name="xlscontactos" method="post" action="xlscontactos.php" target="_blank">
                ...
                <table>
                    ...
                    for ($i = 0; $i < $nflotas ; $i++){
                        ...
                        foreach ($contactos as $tipocont => $idcont){
                            if ($idcont > 0){
                                if (!in_array($idcont, $contactos_flota)){
                                    if ((isset($contactsel)) && ($contactsel != 'NN')){
                                        if ($contactsel == $tipocont){
                        ...
                        foreach ($contactos_flota as $tipocont => $idcont){
                            ...
                            $sql_contflota = "SELECT * FROM contactos_flotas WHERE (CONTACTO_ID = $idcont) AND (FLOTA_ID = $idflota)";
                    ...
                    ?>
                                    <tr <?php if (($idcf % 2) == 0) {echo " class='filapar'";}?>>
                                        <td class="centro"><?php echo $contflota['ID']; ?></td>
