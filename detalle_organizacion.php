<?php
// Obtenemos el idioma de la cookie de JoomFish
$idioma = $_COOKIE['jfcookie']['lang'];
$lang = "idioma/organizadet_$idioma.php";
include ($lang);

// ------------ Conexión a BBDD de Terminales ----------------------------------------- //
include("conexion.php");
$base_datos = $dbbdatos;
$link = mysql_connect($dbserv, $dbusu, $dbpaso);
if (!link) {
    echo "<b>ERROR MySQL:</b>" . mysql_error();
}
else {
    // Seleccionamos la BBDD y codificamos la conexión en UTF-8:
    if (!mysql_select_db($base_datos, $link)) {
        echo 'Error al seleccionar la Base de Datos: ' . mysql_error();
        exit;
    }
    mysql_set_charset('utf8', $link);
}
// ------------------------------------------------------------------------------------- //

// Importamos las variables de formulario:
import_request_variables("p", "");

/*
 *  $permiso = variable de permisos de flota:
 *      0: Sin permiso
 *      1: Permiso de consulta
 *      2: Permiso de modificación (Oficina COMDES)
 */
// Obtenemos el usuario
include_once('auth_user.php');
    
$permiso = 0;
if ($flota_usu == 100) {
    $permiso = 2;
}
?>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
        <title><?php echo $titulo; ?></title>
<?php
        if ($flota_usu == 0){
?>
            <script type="text/javascript">
                window.top.location.href = "https://comdes.gva.es/cvcomdes/";
            </script>
<?php
        }
?>
        <link rel="StyleSheet" type="text/css" href="estilo.css">
    </head>
    <body>
    <?php
    if ($permiso > 1){
        //datos de la tabla Organizaciones:
        $sql_org = "SELECT * FROM organizaciones WHERE ID = " . $idorg;
        $res_org = mysql_query($sql_org) or die("Error en la consulta de Organización: " . mysql_error());
        $norg = mysql_num_rows($res_org);
        if ($norg > 0) {
            $row_org = mysql_fetch_array($res_org);
            $ineorg = $row_org['INE'];
            $sql_munorg = "SELECT * FROM municipios WHERE INE = '$ineorg'";
            $res_munorg = mysql_query($sql_munorg) or die("Error en la consulta de Municipio de la Organización" . mysql_error());
            $nmunorg = mysql_num_rows($res_munorg);
            if ($nmunorg > 0) {
                $row_munorg = mysql_fetch_array($res_munorg);
            }        
        }
        if (isset ($update)){
            if ($update == "KO"){
                $clase = "flashko";
                $imagen = "imagenes/cancelar.png";
                $alt = "Error";
            }
            if ($update == "OK"){
                $clase = "flashok";
                $imagen = "imagenes/okm.png";
                $alt = "OK";
            }
?>
            <p class="<?php echo $clase;?>">
                <img src="<?php echo $imagen;?>" alt="<?php echo $alt;?>" title="<?php echo $alt;?>"> &mdash; <?php echo $mensflash;?>
            </p>
<?php
        }
        $link = "document.orgdet.submit();document.orgdet.action='detalle_organizacion.php'";
        $linkpdf = "document.orgdet.action='pdforganizacion.php';".$link;
        $linkxls = "document.orgdet.action='xlsorganizacion.php';".$link;
?>
        <form name="orgdet" action="detalle_organizacion.php" method="post" target="_blank">
            <input type="hidden" name="idorg" value="<?php echo $idorg; ?>">
        <h1>
            <?php echo $h1;?>  &mdash;
            <input type='image' name='action' src='imagenes/newtab.png' alt='<?php echo $newtab;?>' title="<?php echo $newtab;?>">  &mdash;
            <a href="#" onclick="<?php echo $linkxls; ?>"><img src="imagenes/xls.png" alt="Exportar a Excel" title="Exportar a Excel"></a>
        </h1>
        </form>
        <h2><?php echo $h2organiza;?></h2>
        <?php
        if ($norg > 0){
        ?>  
            <table>
                <tr>
                    <th><?php echo $thorganiza; ?></th>
                    <th><?php echo $thdomicilio; ?></th>
                    <th><?php echo $thcp; ?></th>
                    <th><?php echo $thciudad; ?></th>
                    <th><?php echo $thprovincia; ?></th>
                </tr>
                <tr>
                    <td><?php echo $row_org['ORGANIZACION'];?></td>
                    <td><?php echo $row_org['DOMICILIO'];?></td>
                    <td><?php echo $row_org['CP'];?></td>
                    <td><?php echo $row_munorg['MUNICIPIO'];?></td>                    
                    <td><?php echo $row_munorg['PROVINCIA'];?></td>
                </tr>
            </table>
            <h2><?php echo $h2resp;?></h2>
        <?php
            if ($row_org['RESPONSABLE'] > 0){
                $sql_resp = "SELECT * FROM contactos WHERE ID = " . $row_org['RESPONSABLE'];
                $res_resp = mysql_query($sql_resp) or die("Error en la consulta de Responsable de la Organización" . mysql_error());
                $nresp = mysql_num_rows($res_resp);
                if ($nresp > 0) {
                    $row_resp = mysql_fetch_array($res_resp);
        ?>
                    <table>
                        <tr>
                            <th><?php echo $thnomresp; ?></th>
                            <th><?php echo $thcargo; ?></th>
                            <th><?php echo $thtelef; ?></th>
                            <th><?php echo $thmail; ?></th>
                        </tr>
                        <tr>
                            <td><?php echo $row_resp['NOMBRE'];?></td>
                            <td><?php echo $row_resp['CARGO'];?></td>
                            <td><?php echo $row_resp['TELEFONO'];?></td>
                            <td><?php echo $row_resp['MAIL'];?></td>
                        </tr>
                    </table>
        <?php
                }
                else{
        ?>
                    <p class="error"><?php echo $errnorf;?></p>    
        <?php   
                }
            }
            else{
        ?>
                <p class="error"><?php echo $errnoresp;?></p>
        <?php
            }
        ?>
            <h2><?php echo $h2flotas;?></h2>
        <?php
            // Datos de la tabla flotas;
            $sql_flotas = "SELECT * FROM flotas WHERE ORGANIZACION = " . $idorg;
            $res_flotas = mysql_query($sql_flotas) or die("Error en la consulta de Flotas de la Organización" . mysql_error());
            $nflotas = mysql_num_rows($res_flotas);
            if ($nflotas > 0) {
                $ntermorg = 0;
        ?>
                <form name="formdet" action="detalle_flota.php" method="post">
                    <input type="hidden" name="idflota" id="idflota" value="#">
                </form>
                <table>
                    <tr>
                        <th><?php echo $thnflota; ?></th>
                        <th><?php echo $thflota; ?></th>
                        <th><?php echo $thacro; ?></th>
                        <th><?php echo $thnterm; ?></th>
                        <th><?php echo $thirflota; ?></th>
                    </tr>
                <?php
                    for ($i = 0; $i < $nflotas; $i++){
                        $row_flota = mysql_fetch_array($res_flotas);
                        $sql_term = "SELECT * FROM terminales WHERE FLOTA = " . $row_flota['ID'];
                        $res_term = mysql_query($sql_term) or die("Error en la consulta de Terminales" . mysql_error());
                        $nterm = mysql_num_rows($res_term);
                        $ntermorg = $ntermorg + $nterm;
                        $linkdet = "document.formdet.idflota.value=" . $row_flota['ID'] . ";document.formdet.submit();"
                        
                ?>
                        <tr <?php if(($i % 2) == 1) echo "class = 'filapar'"; ?>>
                            <td class="centro"><?php echo ($i + 1);?></td>
                            <td><?php echo $row_flota['FLOTA']; ?></td>
                            <td><?php echo $row_flota['ACRONIMO']; ?></td>                      
                            <td  class="centro"><?php echo $nterm; ?></td>                            
                            <td class="centro">
                                <a href="#" onclick="<?php echo $linkdet;?>">
                                    <img src="imagenes/ir.png" alt="<?php echo $thirflota; ?>" title="<?php echo $thirflota; ?>">
                                </a>
                            </td>
                        </tr>
                <?php
                    }
                ?>
                    <tr>
                        <th colspan = "3"><?php echo $thtotterm; ?></th>
                        <td class="centro" colspan="2"><?php echo $ntermorg; ?></td>
                    </tr>
                </table>
        
        <?php
            }
            else{
        ?>
            <p class="error"><?php echo $errnoflotas;?></p>
        <?php
            }
        }
        else{
        ?>
            <p class="error"><?php echo $errnoorg;?></p>
        <?php
        }
        ?>
        <form name="accorg" action="detalle_organizacion.php" method="post">
            <input type="hidden" name="idorg" value="<?php echo $idorg; ?>">
        </form>        
        <table>
            <tr>
                <td class="borde">
                    <a href='#' onclick="document.accorg.action='organizaciones.php';document.accorg.submit();">
                        <img src='imagenes/atras.png' alt='<?php echo $botatras; ?>' title='<?php echo $botatras; ?>'>
                    </a><br><?php echo $botatras; ?>
                </td>
            <?php
            if ($norg > 0){
            ?>
                <td class="borde">
                    <a href='#' onclick="document.accorg.action='editar_organiza.php';document.accorg.submit();">
                        <img src='imagenes/pencil.png' alt='<?php echo $botedit; ?>' title='<?php echo $botedit; ?>'>
                    </a><br><?php echo $botedit; ?>
                </td>
            <?php
            }
            ?>
                <td class="borde">
                    <a href='#' onclick="document.accorg.action='contactos_organiza.php';document.accorg.submit();">
                        <img src='imagenes/editacont.png' alt='<?php echo $botcontacto; ?>' title='<?php echo $botcontacto; ?>'>
                    </a><br><?php echo $botcontacto; ?>
                </td>
            </tr>
        </table>
    <?php
    }
    else{
    ?>
        <h1><?php echo $h1noperm;?></h1>
        <p class="error"><?php echo $errnoperm;?></p>
    <?php
    }
    ?>
    </body>
</html>
