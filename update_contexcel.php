    ...
    if ($permiso == 2) {
       // Aumentamos el tamaño de la memoria:
        ...
        if ($nflota == 0){
            ...
            $res_cfcheck = mysql_query($sql_cfcheck) or die("Error en la consulta del Responsable de Flota: " . mysql_error());
            $ncfcheck = mysql_num_rows($res_cfcheck);
            if ($ncfcheck > 0){
                $sql_delcfresp = "DELETE FROM contactos_flotas WHERE (FLOTA_ID = " . $idflota . ") AND (ROL = 'RESPONSABLE')";
                $res_delcfresp = mysql_query($sql_delcfresp) or die("Error al borrar Contacto del Respobsable de Flota: " . mysql_error());
            }
            ...
            if ($filaop > 0){
                ...
                foreach ($contop as $contacto) {
                    ...
                    $sql_inscfop = "INSERT INTO contactos_flotas (CONTACTO_ID, FLOTA_ID, ROL, ORDEN) VALUES";
                    $sql_inscfop .= " (" . $idop . ", " . $idflota . ", 'OPERATIVO', $orden)";
            ...
            if ($filatec > 0){
                ...
                foreach ($contec as $contacto) {
                    ...
                    $sql_inscftec = "INSERT INTO contactos_flotas (CONTACTO_ID, FLOTA_ID, ROL, ORDEN) VALUES";
                    $sql_inscftec .= " (" . $idtec . ", " . $idflota . ", 'TECNICO', $orden)";
            ...
            if ($fila24h > 0){
                ...
                foreach ($cont24h as $contacto) {
                    ...
                    $sql_inscf24h = "INSERT INTO contactos_flotas (CONTACTO_ID, FLOTA_ID, ROL, ORDEN) VALUES";
                    $sql_inscf24h .= " (" . $id24h . ", " . $idflota . ", 'CONT24H', $orden)";
                    $res_inscf24h = mysql_query($sql_inscf24h) or die("Error al agregar el Contacto 24x7: " . mysql_error());
                }
            }
            // Actualizamos el formato:
            if ((isset($formcont)) && ($formcont == 'SI')){
                $sql_updflota = "UPDATE FLOTAS SET FORMCONT = 'SI', UPDCONT = '" . $fecha . "' WHERE (ID = " . $idflota . ")";
                $res_update = mysql_query($sql_updflota) or die("Error al actualizar la Flota : " . mysql_error());
            }
