...
<?php
    if ($permiso == 2) {
        ...
        if ($nflota == 0){
            ...
            $sql_delterm .= ")";
            $res_delterm = mysql_query($sql_delterm) or die ($errissi." $issi: ".mysql_error());
            $ndelterm = mysql_num_rows($res_delterm);
            if ($ndelterm > 0){
                for ($j = 0; $j < $ndelterm; $j++){
                    $row_delterm = mysql_fetch_array($res_delterm);
                    $sql_delterm = "DELETE FROM terminales WHERE ID = '".$row_delterm["ID"]."'";
                    $res_update = mysql_query($sql_delterm) or die ($errdelterm." ".$row_delterm["ISSI"].": ".mysql_error());
                }
            }
        ...
        if ($res_update){
            if (unlink($fichero)){
                $update = "OK";
                $mensflash = sprintf($mensaje, $ninsert, $nupdate, $ndelterm);
            }
            else{
                $update = "KO";
